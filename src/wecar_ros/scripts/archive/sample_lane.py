...
import sample_edge_detection as edge  # Handles the detection of lane lines
...
class Lane:
    ...
    def __init__(self, orig_frame):
        ...
        self.roi_points = np.float32(
            [
                (width / 2 - 150, height / 2 + 20),  # Top-left corner
                (0, height),  # Bottom-left corner
                (width, height),  # Bottom-right corner
                (width / 2 + 150, height / 2 + 20),  # Top-right corner
            ]
        )
    ...
    def get_lane_line_indices_sliding_windows(self, plot=False):
        ...
        if len(lefty) != 0 and len(lefty) != 0:
